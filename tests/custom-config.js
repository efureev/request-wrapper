...
import ActionInterceptor from '../src/interceptors/response/ActionInterceptor'
...
export const customRequestWithInterceptor = (config = {}) => {
  const headers = {
    'X-Requested-With': 'XMLHttpRequest',
    'X-Debug-User': 'test@example.com',
    ...(isObject(config.headers) ? config.headers : {}),
...
describe('create request with interceptor', () => {
  describe('checking Request', () => {
    ...
    it('same config', () => {
      ...
      assert.strictEqual(true, isObject(config.headers))
      assert.strictEqual(0, Object.keys(config.headers).length)
      assert.strictEqual(true, config.isResponseWrap)
      assert.strictEqual(true, isObject(config.responseWrapper))
    })
  })
